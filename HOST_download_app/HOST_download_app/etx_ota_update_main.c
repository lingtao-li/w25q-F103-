...
{
  uint16_t len;
  ETX_OTA_COMMAND_BAK *ota_start = (ETX_OTA_COMMAND_BAK*)DATA_BUF;
  int ex = 0;
  ...
  len = sizeof(ETX_OTA_COMMAND_BAK);

  //send OTA START
  for(int i = 0; i < len; i++)
  {
    delay(1);
    if(1 /*RS232_SendByte(comport, DATA_BUF[i])*/)
    {
      //some data missed.
...
{
  uint16_t len;
  ETX_OTA_COMMAND_BAK *ota_end = (ETX_OTA_COMMAND_BAK*)DATA_BUF;
  int ex = 0;
...
int main(int argc, char *argv[])
{
  ...
  do
  {
    ...
    if(1 /*RS232_OpenComport(comport, bdrate, mode, 0)*/)
    {
